...
// Holds led strip status
static struct led_rgb pixels[NUM_PIXELS];

static inline uint8_t rand8(void) { return (uint8_t)sys_rand32_get(); }
...
static struct led_rgb wheel_color(uint8_t pos) {
  if (pos < 85) {
    return (struct led_rgb){
        .r = (uint8_t)(255 - pos * 3), .g = (uint8_t)(pos * 3), .b = 0};
  } else if (pos < 170) {
    pos -= 85;
    return (struct led_rgb){
        .r = 0, .g = (uint8_t)(255 - pos * 3), .b = (uint8_t)(pos * 3)};
  }

  pos -= 170;
  return (struct led_rgb){
      .r = (uint8_t)(pos * 3), .g = 0, .b = (uint8_t)(255 - pos * 3)};
}
...
static void game_wheel_cycle(const struct led_rgb *color) {
  ...
  for (int offset = 0; offset < NUM_PIXELS; offset++) {
    for (int i = 0; i < NUM_PIXELS; i++) {
      pixels[i] =
          wheel_color((uint8_t)((i * 256 / NUM_PIXELS + offset * 8) & 0xff));
    }
    show_pixels();
    k_sleep(K_MSEC(35));
  }
...
static const light_game_t games[] = {
    game_single_dot,     game_tail_chase,     game_color_wipe,
    game_theater_chase,  game_bounce,         game_blink_all,
    game_alternate,      game_pair_run,       game_sparkle,
    game_gradient_sweep, game_random_scatter, game_segment_run,
    game_pulse_all,      game_wheel_cycle,    game_rainbow,
};

static const char *const game_names[] = {
    "Single Dot", "Tail Chase",     "Color Wipe",     "Theater Chase",
    "Bounce",     "Blink All",      "Alternate",      "Pair Run",
    "Sparkle",    "Gradient Sweep", "Random Scatter", "Segment Run",
    "Pulse All",  "Wheel Cycle",    "Rainbow",
};

int main(void) {
  if (!device_is_ready(strip)) {
    LOG_ERR("Errore: dispositivo LED strip non pronto.\n");
    return -ENODEV;
  }

  LOG_INF("Avvio sequenza di giochi luminosi per %d LED...\n", NUM_PIXELS);
  ...
  while (1) {
    for (int game_idx = 0; game_idx < game_count; game_idx++) {
      ...
      LOG_INF("Gioco %d/%d: %s - colore R:%u G:%u B:%u\n", game_idx + 1,
              game_count, game_names[game_idx], color.r, color.g, color.b);
