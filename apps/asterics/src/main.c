...
int main(void) {
  ...
  const struct device *gyro_prova = DEVICE_DT_GET(DT_NODELABEL(mcu_bmi088_gyro));
...
int ret;
  while (1) {
        ...
        if (ret < 0) {
            ...
            // 6. Stampa formattata
            // Usiamo il formato intero.decimale per evitare problemi con float se non configurati
            LOG_INF("GYRO (rad/s): X=%d.%06d | Y=%d.%06d | Z=%d.%06d",
                    gyro_data[0].val1, abs(gyro_data[0].val2),
                    gyro_data[1].val1, abs(gyro_data[1].val2),
                    gyro_data[2].val1, abs(gyro_data[2].val2));
...
if (ret < 0) {
    ...
    // 4. Stampa i dati. Nota che ogni asse ha la sua parte intera (val1) e frazionaria (val2).
    // Qui stampiamo solo la parte intera (val1) per semplicità, come nel tuo esempio originale.
