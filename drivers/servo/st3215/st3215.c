...
/** @brief Calculates the checksum of the ST3215 packet. The checksum is the
 * bitwise NOT of the sum of the bytes starting from the ID.
 * @param packet The packet for which to calculate the checksum.
 * @param total_length The total length of the packet (including header and
 * checksum).
 * @return The calculated checksum byte.
 */
static uint8_t st3215_calc_checksum(uint8_t *packet, uint8_t total_length) {
  uint32_t sum = 0;
  // sums all the bytes starting from the ID (index 2) up to the last byte
  // before the checksum
...
/**
 * @brief Sends the raw angle command to the ST3215 servo. This function
 * constructs the command packet according to the servo's protocol and sends it
 * over UART
...
int st3215_send_angle_raw(const struct device *dev, uint16_t angle) {
  ...
  uint8_t packet[9];
  // since the angle input parameter is 16-bit, we need to split it into two
  ...
  packet[3] = 0x05; // remaining packet length (5 bytes: Instruction + Address +
                    // Data Length + 2 bytes of angle + Checksum)
  ...
  for (int i = 0; i < 9; i++) {
    uart_poll_out(config->uart_dev,
                  packet[i]); // writes byte by byte to the UART
  }
...
int st3215_get_angle_raw(const struct device *dev, uint16_t *angle) {
  ...
  request[3] = 0x04; // remaining packet length (4 bytes: Instruction + Address
                     // + Data Length + Checksum)
  request[4] = ST3215_INST_READ;
  request[5] = ST3215_REG_PRESENT_POS;
  request[6] = 0x02; // we want to read 2 bytes (the angle is 16-bit) starting
                     // from the position specified in request[5]
  ...
  if (bytes_received < 8) {
    LOG_ERR("Error: timeout receiving data (only %d bytes received)",
            bytes_received);
    return -ETIMEDOUT;
  }
  // response structure: [0xFF, 0xFF, ID, Len, Status, Val_LOW, Val_HIGH,
  // Checksum]
  ...
  if (rx_checksum != response[7]) {
    LOG_ERR("Error: checksum response not valid! (calculated: 0x%02X, "
            "Received: 0x%02X)",
...
#define ST3215_DEFINE(inst)                                                    \
  ...
  static const struct st3215_config st3215_config_##inst = {                   \
      .uart_dev = DEVICE_DT_GET(DT_INST_BUS(inst)),                            \
                                                                               ...
                                                                               \
  DEVICE_DT_INST_DEFINE(inst, st3215_init, NULL, &st3215_data_##inst,          \
                        &st3215_config_##inst, POST_KERNEL,                    \
