                                ...
                                uint32_t timeout_ms)
{
    ...
    if (buffer[0] != ST3215_HEADER || buffer[1] != ST3215_HEADER) {
        return -EIO;
    }
...
static int st3215_ping(const struct device *dev)
{
    ...
    ret = st3215_read_response(dev, response, 7, 50);
    if (ret < 0) {
...
#define ST3215_DEFINE(inst)                                                  \
    ...
    static const struct st3215_config st3215_config_##inst = {               \
        ...
        .time_ms = DT_INST_PROP_OR(inst, time_ms, 500),                      \
    };                                                                       \
